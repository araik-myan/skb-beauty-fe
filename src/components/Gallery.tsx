...
function GalleryImage({
  ...
  const isVisible = useInView(imgRef, { once: true, margin: "-40px" });
  ...
  return (
    <motion.a
      ...
      transition={{ duration: 0.6, delay: 0.05 * (index % 3), ease }}
      className={`group relative overflow-hidden rounded-xl md:rounded-xl cursor-pointer block break-inside-avoid ${heightClass}`}
    >
      <Image
        src={src}
...
export default function Gallery() {
  ...
  return (
    <section id="galerie" className="py-16 md:py-28 lg:py-36 bg-white relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 divider-line" />

      <div ref={ref} className="max-w-7xl mx-auto px-5 md:px-6 lg:px-8">
        {/* Header */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8, ease }}
          className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 md:gap-8 mb-10 md:mb-16 lg:mb-20"
        >
          <div className="max-w-xl">
            ...
            <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl xl:text-6xl text-charcoal leading-[1.1]">
              {t.heading1[lang]}
              <span className="text-camel-dark italic font-light">{t.heading2[lang]}</span>
            </h2>
          </div>
          <p className="text-taupe leading-relaxed text-sm md:text-[15px] max-w-sm">
            {t.description[lang]}
          </p>
        </motion.div>

        {/* Gallery — 2 columns mobile, 3 columns desktop masonry */}
        <div className="columns-2 lg:columns-3 gap-2.5 md:gap-3 lg:gap-4 space-y-2.5 md:space-y-3 lg:space-y-4">
          {images.map((src, index) => (
            <GalleryImage
              key={index}
              src={src}
              index={index}
              overlayGradient={overlayGradients[index]}
              heightClass={heights[index]}
              viewOnInstagram={t.viewOnInstagram[lang]}
            />
          ))}
        </div>
        ...
        >
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group inline-flex items-center gap-3 px-8 py-3.5 md:py-4 border border-charcoal/15 text-charcoal rounded-full text-[11px] md:text-[12px] uppercase tracking-[0.2em] hover:bg-charcoal hover:text-cream-light transition-all duration-500"
          >
            {t.followUs[lang]}
            <ArrowUpRight
              size={14}
